...
function toggle() {
    const fs = top.document.getElementsByTagName('frameset')[0];
    const f = fs.getElementsByTagName('frame');
    if (collapsed) {
        fs.rows = '300px,*';
...
T = M.sin;
E = M.sqrt;
for (O = k = 0; x = z = j = i = k < 250;) with (M[k] = k ? c.cloneNode(0) : c) {
    width = height = k ? 32 : W = 800;
    with (getContext('2d')) if (k > 10 | !k) for (font = '60px Impact', V = 'rgba('; I = i * U, fillStyle = k ? k == 13 ? V + '205,205,215,.15)' : V + (147 + I) + ',' + (k % 2 ? 128 + I : 0) + ',' + I + ',.5)' : '#55ff00', i < 7;) beginPath(fill(arc(U - i / 3, 24 - i / 2, k == 13 ? 4 - (i++) / 2 : 8 - i++, 0, M.PI * 2, 1))); else for (; x = T(i), y = Q() * 2 - 1, D = x * x + y * y, B = E(D - x / .9 - 1.5 * y + 1), R = 67 * (B + 1) * (L = k / 9 + .8) >> 1, i++ < W;) if (D < 1) beginPath(strokeStyle = V + R + ',' + (R + B * L >> 0) + ',40,.1)'), moveTo(U + x * 8, U + y * 8), lineTo(U + x * U, U + y * U), stroke();
